...
export default function MediaUpload ({
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      // Validate file type
      const allowedTypes =
        type === 'image' ? ALLOWED_MIME_TYPES.IMAGE : ALLOWED_MIME_TYPES.VIDEO
      ...
      // Validate file size
      const maxSize =
        type === 'image' ? FILE_SIZE_LIMITS.IMAGE : FILE_SIZE_LIMITS.VIDEO
      ...
      const fileName = `${Date.now()}-${Math.random()
        ...
        .substring(2)}.${fileExt}`
      const bucketName =
        type === 'image' ? STORAGE_BUCKETS.IMAGES : STORAGE_BUCKETS.VIDEOS
